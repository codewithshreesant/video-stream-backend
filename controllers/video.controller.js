...
const uploadVideo = asyncHandler(async (req, res) => {
  ...
  const videoFile = req.files?.video?.[0]; // Now the file object from memoryStorage
  const thumbnailFile = req.files?.thumbnail?.[0]; // Now the file object from memoryStorage
  const uploader = req.user?._id;
  console.log("video file ", videoFile);
  console.log("image file  ", thumbnailFile);

  if (!title || !videoFile?.buffer || !thumbnailFile?.buffer || !uploader) {
    return res
      .status(400)
      .json(new ApiResponse(400, 'Missing required fields or files'));
  }

  try {
    const videoResult = await uploadToCloudinary(videoFile.buffer, { // Pass the buffer
      resource_type: 'video',
      folder: 'videos',
    });

    const thumbnailResult = await uploadToCloudinary(thumbnailFile.buffer, { // Pass the buffer
      folder: 'thumbnails',
    });
    ...
    const video = await Video.create({
      ...
      cloudinaryVideoId: videoResult.public_id, // Store Cloudinary public ID for deletion
      cloudinaryThumbnailId: thumbnailResult.public_id, // Store Cloudinary public ID for deletion
    });

    // No need to unlink files from local storage when using memoryStorage
